...
interface UpcomingElection {
  year: string; // "令和7年度", "令和8年度", "令和9年度以降"
  yearLabel: string; // "令和7年", "令和8年", "令和9年"
  name: string;
  notice: string;
  day: string; // "10月25日" 形式
  isoDate: string; // "2026-10-25"
}
...
// 新潟県選挙管理委員会「県内選挙スケジュール」
// 出典: https://www.pref.niigata.lg.jp/site/senkyo/
...
// 令和7年度(2025)、令和8年度(2026)、令和9年度以降(2027〜)の年度別選挙スケジュール
...
interface AppState {
  ...
  selectedElectionYear: string; // "すべて", "令和7年度", "令和8年度", "令和9年度以降"
}
...
const state: AppState = {
  tab: "home",
  electionDate: "2026-10-25", // 初期値: 新潟市長選挙
  quizStep: 0,
  scores: freshScores(),
  quizFinished: false,
  selectedWard: "すべて",
...
// ---------- Tiny icon helper (dependency-free, lucide-inspired strokes) ----------
...
function renderHome(): HTMLElement {
  ...
  // 年度切替フィルターボタン (令和7年度 / 令和8年度 / 令和9年度以降 / すべて)
...
function renderPlace(): HTMLElement {
  ...
  tipsCard.innerHTML = `
    <p class="tips-title">💡 投票へ行くときのポイント</p>
    <ul class="tips-list">
  ...
  NIIGATA_WARDS.forEach((w) => {
    const count = w === "すべて" 
      ...
      : POLLING_PLACES.filter(p => p.wardShort === w).length;
  ...
  function updatePollingListContainer() {
    ...
    countBadge.textContent = q
      ? `🔍 「${state.selectedWard}」の検索結果: ${filtered.length}件の投票所が見つかりました`
      : `📍 「${state.selectedWard}」: 全${filtered.length}件の投票所を表示中`;
  ...
  footnote.className = "footnote";
  footnote.textContent =
    "※掲載データは新潟市公式「投票所一覧（令和8年5月1日時点）」に基づいています。投票所は住民登録住所によって指定されます。投票所入場券に記載の場所が正式な投票場所です。";
  wrap.appendChild(footnote);
